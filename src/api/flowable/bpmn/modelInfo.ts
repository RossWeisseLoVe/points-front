...
enum Api {
  ...
  ValidateBpmnModel = '/flow/bpmnDesigner/open/api/validateBpmnModel',
  SaveBpmnModel = '/flow/bpmnDesigner/open/api/saveBpmnModel',
}

export const validateBpmnModel = (params: Object) =>
  defHttp.post<any>({ url: Api.ValidateBpmnModel, params }, {isReturnNativeResponse: true}).then(res=>{
    return Promise.resolve(res.data);
  });

export const saveBpmnModel = (params: Object) =>
  defHttp.post<any>({ url: Api.SaveBpmnModel, params }, {isReturnNativeResponse: true}).then(res=>{
    return Promise.resolve(res.data);
  });
